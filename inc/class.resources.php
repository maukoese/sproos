...
class _hResources {
  ...
  function createResource() {
    ...
    $h_link = hADMIN."resource?view=$h_code";
  ...
  function updateResource( $h_code) {
    ...
    if ( mysqli_query( $GLOBALS['conn'], "UPDATE hresources SET h_alias = '".$h_alias."', h_author = '".$h_author."', h_avatar = '".$h_avatar."', h_organization = '".$h_organization."', h_code = '".$h_code."', h_created = '".$h_created."', h_description = '".$h_description."', h_email = '".$h_email."', h_gender = '".$h_gender."', h_key = '".$h_key."', h_level = '".$h_level."', h_link = '".$h_link."', h_location = '".$h_location."', h_notes = '".$h_notes."', h_password = '".$h_password."', h_phone = '".$h_phone."', h_type = '".$h_type."' WHERE h_code = '".$h_code."'" ) ) {
      echo "<script type = \"text/javascript\">
  ...
  <i class="material-icons">edit</i></a>
      ...
      while ( $resourcesDetails = mysqli_fetch_assoc( $getResourcesBy)){
        ...
        <td class="mdl-data-table__cell--non-numeric">
        <a href="./resource?view=<?php _show_( $resourcesDetails['h_code'] ); ?>&key=<?php _show_( $resourcesDetails['h_alias'] ); ?>" ><i class="material-icons">perm_identity</i></a> 
  ...
  function getResourcesAuthor( $author) { 
    ...
    if ( $getResourcesBy -> num_rows > 0) {
      ...
      while ( $usersDetails = mysqli_fetch_assoc( $getResourcesBy)){
        ...
        <td class="mdl-data-table__cell--non-numeric">
        <a href="./resource?view=<?php _show_( $usersDetails['h_code'] ); ?>&key=<?php _show_( $usersDetails['h_alias'] ); ?>" ><i class="material-icons">perm_identity</i></a> 
  ...
  <i class="material-icons">create</i></a>
      ...
      <div class="mdl-cell--12-col mdl-grid">
      <table id="mdl-table" class="mdl-data-table mdl-js-data-table mdl-data-table--selectable sort mdl-shadow--2dp mdl-color--<?php primaryColor(); ?>">
        <thead>
        <tr>
  ...
  function getResourceCode( $code) {
    ...
    if ( $getResourceCode -> num_rows > 0) {
      while ( $resourceDetails = mysqli_fetch_assoc( $getResourceCode)){
        ...
        ?><title><?php _show_( $resourceDetails['h_alias'] ); ?> [ <?php showOption( 'name' ); ?> ]</title>
        <div class="mdl-grid" >
                    ...
                    <div class="mdl-card mdl-shadow--2dp mdl-color--<?php primaryColor(); ?>">
                        ...
                        <div class="mdl-card__supporting-text mdl-card--expand mdl-grid">
                          <div class="mdl-cell mdl-cell--8-col-desktop mdl-cell--8-col-tablet mdl-cell--12-col-phone">
                            ...
                            <h5>
                            <h6><b>Admin Email:</b> <a href="mailto:<?php _show_( $resourceDetails['h_email'] ); ?>"><?php _show_( $resourceDetails['h_email'] ); ?></a><br><?php if ( $resourceDetails['h_type'] !== "organization" ) { ?>
                            <b>Center:</b> <a href="./resource?center=<?php _show_( $resourceDetails['h_organization'] ); ?>"><?php _show_( $resourceDetails['h_organization'] ); ?></a><br><?php } 
                ...
                <div class="mdl-cell mdl-cell--4-col-desktop mdl-cell--4-col-tablet mdl-cell--12-col-phone">
                    <div class="mdl-card mdl-shadow--2dp mdl-color--<?php primaryColor(); ?>">
                        ...
                        <i class="material-icons">local_hospital</i>
                          <span class="mdl-button"><?php _show_( $resourceDetails['h_organization'] ); ?></span>
                            <div class="mdl-layout-spacer"></div>
